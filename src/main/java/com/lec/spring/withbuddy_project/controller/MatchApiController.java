...
import com.lec.spring.withbuddy_project.service.MatchService;
...
//    private final SimpMessagingTemplate messagingTemplate;
    ...
    public int matchSave(@RequestParam(name = "senderId") Long senderId, @RequestParam(name = "receiverId") Long receiverId) {
        ...
        int save = matchService.save(senderId, receiverId); //매칭하기 버튼을 누르면 매칭을 만듬
        boolean match = matchService.ifExist(senderId, receiverId); // 만약 매칭이 있다면 0을 리턴 아니면 1을 리턴해서 매칭을 만들지 말지 결정
    ...
    public List<MatchResponse> matchList(@PathVariable(name = "userId") Long userId) {
        List<MatchResponse> list = matchService.list(userId); // "userId==로그인한 유저의 아이디" 에 따른 매칭 리스트를 렌더링하기 위한 정보
    ...
    public void matchAccept(@RequestParam(name = "senderId") Long senderId, @RequestParam(name = "receiverId") Long receiverId) {
        ...
        MatchRequest match2 = MatchRequest.builder()
                ...
                .build();
        matchService.acceptMatch(match);  // accept여부를 true로 업데이트
    ...
    public void matchDelete(@RequestParam(name = "senderId") Long senderId,@RequestParam(name = "receiverId") Long receiverId) {
        ...
        MatchRequest match2 = MatchRequest.builder()
                ...
                .receiverId(senderId)
                .build();
        matchService.delete(match1);  // 매칭 수락을 거절하면 delete함
